...
public class RoundGroupMenuItem extends AbstractGroupMenuItem {
    ...
    protected void renderContent(DrawContext context) {
        context.drawText(MinecraftClient.getInstance().textRenderer, Text.translatable(groupData.name).formatted(Formatting.ITALIC), 0, 0, 0xFFFFFF, true);
    }
    ...
    protected void open() {
        for (int i = 0; i < groupData.list.size(); i++) {
            ...
            var childPos = new Vec2f((float) Math.cos(angleRad), (float) Math.sin(angleRad)).multiply(100);
