...
public class RoundGroupMenuItem extends AbstractGroupMenuItem {
    ...
    protected void renderContent(DrawContext context, Vec2f mousePos) {
        context.drawText(MinecraftClient.getInstance().textRenderer, Text.translatable(groupData.name).formatted(Formatting.ITALIC), (int) pos.x, (int) pos.y, 0xFFFFFF, true);
    }
    ...
    protected void open() {
        for (int i = 0; i < groupData.list.size(); i++) {
            ...
            var childPos = pos.add(new Vec2f((float) Math.cos(angleRad), (float) Math.sin(angleRad)).multiply(100));
